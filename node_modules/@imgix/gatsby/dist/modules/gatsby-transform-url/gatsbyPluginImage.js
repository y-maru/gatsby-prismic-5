...
function getGatsbyImageData(_a) {
    ...
    if (layout == null) {
        throw new Error("[@imgix/gatsby] A valid 'layout' is required. Valid image layouts are 'fixed', 'fullWidth', and 'constrained'. Found ".concat(layout));
    }
    var width = props.width;
    if (layout === 'fixed' && width == null) {
        throw new Error("[@imgix/gatsby] a 'width' is required when layout is 'fixed'. Found ".concat(width));
    }
    ...
    var client = (0, common_1.createImgixClient)({
        domain: (0, uri_1.parseHost)(src),
        libraryParam: 'gatsbyHook',
    });
    var breakpointsData = (0, breakpoints_1.generateBreakpoints)(__assign(__assign({}, (layout === 'fullWidth'
        ? { layout: 'fullWidth' }
        : { layout: layout, width: width })), { srcsetMinWidth: srcsetMinWidth, srcsetMaxWidth: srcsetMaxWidth, widthTolerance: widthTolerance, sourceWidth: sourceWidth, disableVariableQuality: disableVariableQuality }));
    var _b = calculateSourceWidthAndHeight({
        sourceWidth: sourceWidth,
        sourceHeight: sourceHeight,
    }), sourceWidthOverride = _b.sourceWidth, sourceHeightOverride = _b.sourceHeight;
    return (0, gatsby_plugin_image_1.getImageData)(__assign(__assign(__assign({ baseUrl: (0, uri_1.parsePath)(src), sourceWidth: sourceWidthOverride, sourceHeight: sourceHeightOverride, aspectRatio: aspectRatio, urlBuilder: urlBuilder(client), pluginName: '@imgix/gatsby', formats: ['auto'], breakpoints: breakpointsOverride !== null && breakpointsOverride !== void 0 ? breakpointsOverride : breakpointsData.breakpoints }, props), { outputPixelDensities: breakpointsData.outputPixelDensities }), { layout: layout, options: {
